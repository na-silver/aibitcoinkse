...
# 환경변수 로드 (파일이 없어도 안전)
load_dotenv('key.env', verbose=False)
...
def get_real_account_balance():
    ...
    try:
        ...
        if isinstance(balances, str) or not balances:
            return {'status': 'api_error', 'error': str(balances)}
        ...
        if current_price:
            ...
            account_data['btc_value_krw'] = 0
            account_data['total_assets'] = account_data['KRW']
...
def render_real_time_status():
    ...
    with col2:
        if account_data['status'] == 'success':
            ...
            """, unsafe_allow_html=True)
        elif account_data['status'] == 'demo_mode':
            st.markdown(f"""
            <div class="metric-card warning-card">
                <h3>💰 계좌 (데모)</h3>
                <p class="status-offline">● DEMO MODE</p>
                <p>API 키 없음</p>
            </div>
            """, unsafe_allow_html=True)
        else:
...
def render_live_portfolio():
    ...
    if account_data['status'] == 'demo_mode':
        st.warning("🎭 **데모 모드**: API 키가 설정되지 않아 실제 계좌 데이터를 표시할 수 없습니다.")
        st.info("💡 **실제 포트폴리오 확인**: 로컬에서 API 키를 설정하여 실행하세요.")
        st.info("📊 **거래 기록**: 하단의 '거래 성과' 탭에서 실제 거래 내역을 확인할 수 있습니다.")
        return
    elif account_data['status'] != 'success':
        st.error("❌ 계좌 데이터를 불러올 수 없습니다.")
        st.info("💡 API 연결 상태를 확인하세요.")
        return
