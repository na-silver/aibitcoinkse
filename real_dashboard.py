...
def calculate_performance_metrics(trades_df):
    ...
    try:
        ...
        if portfolio_data:
            ...
            current_total_value = portfolio_data[0]['total_value']
            
            # 초기 자본에서 현재 KRW를 빼면 실제 투자한 금액
            # 포트폴리오 히스토리에서 초기 KRW 가져오기
            all_portfolio = db.get_portfolio_history(365)  # 1년치 데이터
            if all_portfolio:
                initial_krw = all_portfolio[-1]['krw_balance']  # 가장 오래된 데이터
                actual_investment = initial_krw - current_krw
                
                # 현재 BTC 가치 계산
                current_btc_value = current_btc * current_btc_price
                
                # 순손익 = 현재 BTC 가치 - 실제 투자한 금액
                net_profit = current_btc_value - actual_investment
                roi = (net_profit / actual_investment * 100) if actual_investment > 0 else 0
                
                total_buy_value = actual_investment  # 실제 투자금으로 업데이트
            else:
                # 백업 계산
                net_profit = 0
                roi = 0
...
def render_portfolio_details(portfolio_df):
    """상세한 포트폴리오 정보 표시"""
    st.markdown('''
    <div class="metric-container">
        <h2 class="section-header">💼 포트폴리오 현황</h2>
    </div>
    ''', unsafe_allow_html=True)
    ...
    # 필요한 정보 추출
    current_krw = latest_portfolio['krw_balance']
    current_btc = latest_portfolio['btc_balance']
    avg_buy_price_from_db = latest_portfolio.get('btc_avg_price', 0)
    total_value = latest_portfolio['total_value']
    
    # 초기 자본 계산 (가장 오래된 데이터에서)
    initial_krw = portfolio_df.iloc[0]['krw_balance']
    
    # 투자 금액 계산
    invested_krw = initial_krw - current_krw  # 실제 투자한 KRW
    
    # 매수 평균가 계산 (실제 투자금액 / 보유 비트코인 수량)
    avg_buy_price = (invested_krw / current_btc) if current_btc > 0 else 0
    
    # 현재 BTC 시세를 가져오기 (최근 trading_logs에서)
    try:
        db = TradingDatabase("trading_enhanced.db")
        recent_logs = db.get_recent_logs(1)
        current_btc_price = recent_logs[0]['current_price'] if recent_logs else avg_buy_price_from_db
    except:
        current_btc_price = avg_buy_price_from_db
    
    # 현재 BTC 가치 계산
    current_btc_value = current_btc * current_btc_price
    
    # 손익 계산
    profit_loss = current_btc_value - invested_krw
    roi = (profit_loss / invested_krw * 100) if invested_krw > 0 else 0
    ...
    with col1:
        st.markdown(f'''
        <div class="stat-box info-card" style="padding: 1.5rem;">
            <div style="text-align: center;">
                <div style="font-size: 0.9rem; color: #b0b0b0; margin-bottom: 0.5rem;">총 보유자산</div>
                <div style="font-size: 2rem; font-weight: bold; color: #ffffff;">₩{total_value:,.0f}</div>
            </div>
        </div>
        ''', unsafe_allow_html=True)
    ...
    with col2:
        ...
        <div class="stat-box info-card" style="padding: 1.5rem;">
            <div style="text-align: center;">
                <div style="font-size: 0.9rem; color: #b0b0b0; margin-bottom: 0.5rem;">보유 KRW</div>
                <div style="font-size: 2rem; font-weight: bold; color: #00D084;">₩{current_krw:,.0f}</div>
            </div>
        </div>
        ''', unsafe_allow_html=True)
    ...
    <div style="margin: 1rem 0;">
        <h3 style="color: #FFA500; margin-bottom: 1rem;">₿ 비트코인 (BTC)</h3>
    </div>
    ''', unsafe_allow_html=True)
    ...
    with col1:
        profit_color = "#00D084" if profit_loss >= 0 else "#FF4757"
        profit_icon = "+" if profit_loss >= 0 else ""
        st.markdown(f'''
        <div class="stat-box {"success-card" if profit_loss >= 0 else "danger-card"}" style="padding: 1.5rem;">
            <div style="text-align: center;">
                <div style="font-size: 0.9rem; color: #b0b0b0; margin-bottom: 0.5rem;">평가손익</div>
                <div style="font-size: 1.8rem; font-weight: bold; color: {profit_color};">{profit_icon}₩{profit_loss:,.0f}</div>
                <div style="font-size: 1.2rem; color: {profit_color}; margin-top: 0.3rem;">{profit_icon}{roi:.2f}%</div>
            </div>
        </div>
        ''', unsafe_allow_html=True)
    ...
    with col2:
        ...
        <div class="stat-box info-card" style="padding: 1.5rem;">
            <div style="text-align: center;">
                <div style="font-size: 0.9rem; color: #b0b0b0; margin-bottom: 0.5rem;">평가금액</div>
                <div style="font-size: 1.8rem; font-weight: bold; color: #FFA500;">₩{current_btc_value:,.0f}</div>
                <div style="font-size: 1rem; color: #b0b0b0; margin-top: 0.3rem;">보유수량: {current_btc:.8f} BTC</div>
            </div>
        </div>
        ''', unsafe_allow_html=True)
    ...
    with col1:
        st.markdown(f'''
        <div class="stat-box info-card" style="padding: 1rem;">
            <div style="text-align: center;">
                <div style="font-size: 0.8rem; color: #b0b0b0; margin-bottom: 0.3rem;">매수금액</div>
                <div style="font-size: 1.3rem; font-weight: bold; color: #ffffff;">₩{invested_krw:,.0f}</div>
            </div>
        </div>
        ''', unsafe_allow_html=True)
    ...
    with col2:
        ...
        <div class="stat-box info-card" style="padding: 1rem;">
            <div style="text-align: center;">
                <div style="font-size: 0.8rem; color: #b0b0b0; margin-bottom: 0.3rem;">매수평균가</div>
                <div style="font-size: 1.3rem; font-weight: bold; color: #ffffff;">₩{avg_buy_price:,.0f}</div>
            </div>
        </div>
        ''', unsafe_allow_html=True)
    ...
    # 현재 시세 정보
    st.markdown(f'''
    <div class="stat-box info-card" style="padding: 1rem; margin-top: 1rem;">
        <div style="text-align: center;">
            <div style="font-size: 0.9rem; color: #b0b0b0; margin-bottom: 0.5rem;">현재 BTC 시세</div>
            <div style="font-size: 1.5rem; font-weight: bold; color: #FFA500;">₩{current_btc_price:,.0f}</div>
        </div>
    </div>
    ''', unsafe_allow_html=True)
    ...
    st.markdown('''
    <div style="margin: 1rem 0;">
        <h3 style="color: #667eea; margin-bottom: 1rem;">📊 포트폴리오 비율</h3>
    </div>
    ''', unsafe_allow_html=True)
    
    # 비율 계산
    krw_ratio = (current_krw / total_value * 100) if total_value > 0 else 0
    btc_ratio = (current_btc_value / total_value * 100) if total_value > 0 else 0
    
    # 파이차트
    fig = go.Figure(data=[go.Pie(
        labels=['BTC', 'KRW'],
        values=[current_btc_value, current_krw],
        hole=.4,
        marker_colors=['#FFA500', '#00D084'],
...
def render_portfolio_chart(portfolio_df):
    ...
    # 실제 투자 금액 계산 (초기 KRW 잔고 - 현재 KRW 잔고)
    if len(portfolio_df) > 0:
        initial_krw = portfolio_df.iloc[0]['krw_balance']  # 초기 KRW 잔고
        current_krw = portfolio_df.iloc[-1]['krw_balance']  # 현재 KRW 잔고
        actual_investment = initial_krw - current_krw  # 실제 투자한 KRW 금액
        initial_investment = actual_investment if actual_investment > 0 else portfolio_df.iloc[0]['total_value']
    else:
        initial_investment = 1000000
    
    # 일별 손익 계산 (현재 포트폴리오 가치 - 투자 원금 - KRW 잔고)
    portfolio_df['invested_amount'] = initial_investment
    portfolio_df['current_value'] = portfolio_df['total_value'] - portfolio_df['krw_balance']  # 순 BTC 가치
    portfolio_df['daily_profit'] = portfolio_df['current_value'] - initial_investment
...
def render_profit_loss_chart(portfolio_df):
    ...
    # 손익 계산 (현재 BTC 가치 - 투자 원금)
    portfolio_df['invested_amount'] = initial_investment
    portfolio_df['current_btc_value'] = portfolio_df['total_value'] - portfolio_df['krw_balance']  # 순 BTC 가치
    portfolio_df['daily_profit'] = portfolio_df['current_btc_value'] - initial_investment
    portfolio_df['daily_roi'] = (portfolio_df['daily_profit'] / initial_investment * 100).round(2)
    ...
    for section_info in section_patterns:
        ...
        for match in matches:
            ...
            if content and len(content) > 10:
                ...
                # 소셜 분석 관련 용어들 포맷팅
                content = content.replace('감정 점수:', '\n• 감정 점수: ')
                content = content.replace('신뢰도:', '\n• 신뢰도: ')
                content = content.replace('강세 비율:', '\n• 강세 비율: ')
                content = content.replace('약세 비율:', '\n• 약세 비율: ')
                content = content.replace('중립 비율:', '\n• 중립 비율: ')
                content = content.replace('조회수:', '\n• 조회수: ')
                content = content.replace('영상 수:', '\n• 영상 수: ')
                content = content.replace('트렌딩 토픽:', '\n• 트렌딩 토픽: ')
                content = content.replace('인기 채널:', '\n• 인기 채널: ')
                
                # 소셜 분석 섹션은 더 많은 내용 표시 (특히 감정 분석)
                if section_info['title'] == '소셜 분석':
                    # 소셜 분석은 500자까지 표시
                    if len(content) > 500:
                        content = content[:500] + '...\n\n[더 많은 내용 보기]'
                else:
        ...
        if len(parts) > 8:
            ...
            for i in range(4):
                ...
                if section_text.strip():
                    # 소셜 분석 관련 내용이 있는 경우 더 길게 표시
                    max_length = 300 if '감정' in section_text or '유튜브' in section_text or '소셜' in section_text else 200
                    content = section_text[:max_length] + '...' if len(section_text) > max_length else section_text
...
def render_ai_analysis_detailed(ai_logs_df):
    ...
    for i, row in filtered_df.head(10).iterrows():
        ...
        # 길이 제한 (소셜 분석 부분은 더 길게 허용)
        if len(clean_reason) > 2500:
            clean_reason = clean_reason[:2500] + "...\n\n[내용이 너무 길어 일부만 표시됩니다]"
                ...
                for i in range(0, len(analysis_sections), 4):
                    ...
                    for j in range(4):
                        if i + j < len(analysis_sections):
                            ...
                            with cols[j]:
                                # 소셜 분석 카드는 더 높게 설정
                                max_height = "800px" if section['title'] == '소셜 분석' else "600px"
                                min_height = "250px" if section['title'] == '소셜 분석' else "200px"
...
def main():
    ...
    with tab2:
        render_portfolio_details(portfolio_df)
